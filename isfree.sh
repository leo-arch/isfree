...
they fall short of being free, we consider them all equally unethical (...)\n" && exit 0;;
	...
	-o|--official) 
		...
		for (( i=0;i<${#inst_packs[@]};i++ )); do
			if [[ $(cat $tmp_file | grep ^"${inst_packs[$i]}":) ]]; then
				if ! [[ $(pacman -Qi ${inst_packs[$i]} | grep -A3 Version | grep 'parabola\|gnu\|nonfree\|Libre') ]]; then
					counter=$((counter+1))
	...
	-a|--aur) 
		...
		for (( i=0;i<${#aur_packs[@]};i++ )); do
			if [[ $(cat $tmp_file | grep ^"${aur_packs[$i]}":) ]]; then
				counter=$((counter+1))
	...
	-r|--repo)
		...
		for (( i=0;i<${#repo_packs[@]};i++ )); do
			if [[ $(cat $tmp_file | grep ^"${repo_packs[$i]}":) ]]; then
				counter=$((counter+1))
	...
	-p|--privacy)
		...
		for (( i=0;i<${#packs[@]};i++ )); do
			if [[ $(cat $tmp_file | grep ^"${packs[$i]}":) ]]; then
				counter=$((counter+1))
